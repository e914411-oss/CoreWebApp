using System;
using System.Collections.Generic;

namespace CoreWebApp.Models.ECRS;

public partial class vw_Idx_FTR_Main_Place2
{
    public int Id { get; set; }

    public DateTime? DataDate { get; set; }

    public string? APPLNO_CUST { get; set; }

    public string? APPL_NO { get; set; }

    public string? APPL_REGCNAME { get; set; }

    public string? APPL_NAME { get; set; }

    public string? BRAND { get; set; }

    public string? APPL_DATE_Idx { get; set; }

    public string? IMP_DATE_Idx { get; set; }

    public string? Accept_Date_Idx { get; set; }

    public string? SUBMIT_DATE_Idx { get; set; }

    public string? APPL_DATE_Show { get; set; }

    public string? IMP_DATE_Show { get; set; }

    public string? Accept_Date_Show { get; set; }

    public string? SUBMIT_DATE_Show { get; set; }

    public string? APPL_DATE { get; set; }

    public string? IMP_DATE { get; set; }

    public string? Accept_Date { get; set; }

    public string? INSP_DATE { get; set; }

    public string? INSP_DATE_Idx { get; set; }

    public string? INSP_DATE_Show { get; set; }

    public string? INSP_RESULT { get; set; }

    public string? INSP_RESULT_Name { get; set; }

    public string? SUBMIT_Reason { get; set; }

    public string? Expr1 { get; set; }

    public string? CONTACT_FAX { get; set; }

    public string? CONTACT_CELLPHONE { get; set; }

    public string? REMARK { get; set; }

    public string? SUBMIT_DATE { get; set; }

    public string? GOODS_ST_DATE { get; set; }

    public string? GOODS_IN_DATE { get; set; }

    public string? GOODS_GO_DATE { get; set; }

    public string? GOODS_ST_DATE_Show { get; set; }

    public string? GOODS_IN_DATE_Show { get; set; }

    public string? GOODS_GO_DATE_Show { get; set; }

    public string? SUBMIT_PS_CODE { get; set; }

    public string? SUBMIT_PS_CODE_Name { get; set; }

    public string? USER_ID { get; set; }

    public string? MOD_DATE { get; set; }

    public string? MOD_TIME { get; set; }

    public int IdxId { get; set; }

    public string? IdxName { get; set; }

    public string? RETURN_DATE_UPDATE { get; set; }

    public string? RETURN_RESULT { get; set; }

    public string? GOODS_ST_CITY { get; set; }

    public string? APPL_REGNO { get; set; }

    public string? SUBMIT_NO7_1 { get; set; }

    public string? GOODS_ST_CITY_PMDS { get; set; }

    public string? RETURN_RESULT_Name { get; set; }

    public string? IsDel { get; set; }

    public string? PLACE_REMARK { get; set; }

    public string? ToADDRESS { get; set; }

    public string? GOODSNAME { get; set; }

    public string? IN_AMT { get; set; }

    public string? CONTACT_MAN { get; set; }

    public string? CONTACT_TEL { get; set; }
}
