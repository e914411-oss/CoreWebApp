using System;
using System.Collections.Generic;

namespace CoreWebApp.Models.ECRS;

public partial class 非稽不可_業者資料
{
    public int 業者編號 { get; set; }

    public string? 市招名稱 { get; set; }

    public string? 商業登記名稱 { get; set; }

    public string? 統一編號 { get; set; }

    public string? 商業登記證號 { get; set; }

    public string? 工廠登記證號 { get; set; }

    public string? 營業縣市 { get; set; }

    public string? 營業鄉鎮區 { get; set; }

    public string? 營業地址 { get; set; }

    public string? 營登縣市 { get; set; }

    public string? 營登鄉鎮區 { get; set; }

    public string? 營登地址 { get; set; }

    public string? 電話 { get; set; }

    public string? 傳真 { get; set; }

    public string? 營業狀態 { get; set; }

    public string? 辦理廚證人數 { get; set; }

    public string? 丙級烹飪證照 { get; set; }

    public string? 乙級烹飪證照 { get; set; }

    public string? 從業人數 { get; set; }

    public string? 證照HACCP { get; set; }

    public string? 證照GMP { get; set; }

    public string? 證照GSP { get; set; }

    public string? 證照CAS { get; set; }

    public string? 其他 { get; set; }

    public string? 負責人姓名 { get; set; }

    public string? 手機 { get; set; }

    public string? 電子郵件 { get; set; }

    public string? 戶籍城市 { get; set; }

    public string? 戶籍鄉鎮區 { get; set; }

    public string? 戶籍地址 { get; set; }

    public string? 通訊城市 { get; set; }

    public string? 通訊鄉鎮區 { get; set; }

    public string? 通訊地址 { get; set; }

    public string? 食品衛生管理員 { get; set; }

    public string? 食品衛生負責人 { get; set; }

    public string? 備註 { get; set; }

    public string? 水源別 { get; set; }

    public string? 水源別縣市 { get; set; }

    public string? 水源別鄉鎮區 { get; set; }

    public string? 水源別地址 { get; set; }

    public string? 水源產品名稱 { get; set; }

    public DateTime? 水源有效日期 { get; set; }

    public string? 水源有效日期民國年 { get; set; }

    public string? 介面異動者 { get; set; }

    public DateTime? 介面異動時間 { get; set; }

    public DateTime? 建檔日 { get; set; }

    public string? 建檔日民國 { get; set; }

    public string? 所屬縣市 { get; set; }

    public string? 建檔縣市 { get; set; }

    public DateTime? 資料傳送時間 { get; set; }

    public string? 資料狀態 { get; set; }

    public string? 最近稽查日 { get; set; }

    public string? 異動人員身分證號 { get; set; }

    public string? 刪除註記 { get; set; }

    public string? 食品技師姓名 { get; set; }

    public string? 技師證號 { get; set; }

    public string? 衛生管理人員 { get; set; }

    public string? 衛生專職人員 { get; set; }

    public string? 營養師 { get; set; }

    public string? 是否為健康食品工廠業者 { get; set; }

    public string? 是否為健康食品工廠業者_刪除註記 { get; set; }

    public string? 廚師人數 { get; set; }

    public string? 刪除人員 { get; set; }

    public DateTime? 刪除時間 { get; set; }
}
