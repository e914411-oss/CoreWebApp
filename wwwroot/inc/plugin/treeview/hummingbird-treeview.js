(function($) {
	...
	$.fn.hummingbird = function(options) {
		...
		if (typeof(methodName) == "undefined") {
			return this.each(function() {
				//-------------------options-------------------------------------------------------//
				//change symbols
				if (options.collapsedSymbol != "icon-plus-circle") {
					$(this).find("i").removeClass("icon-plus-circle").addClass(options.collapsedSymbol);
				}
	...
	$.fn.hummingbird.defaults = {
		expandedSymbol: "icon-circle-minus",
		collapsedSymbol: "icon-plus-circle",
		collapseAll: true,
		checkboxes: "enabled",
		checkDoubles: false,
		checkDisabled: false,
	...
	$.fn.hummingbird.disableNode = function(tree, attr, name, state) {
		...
		this_checkbox.parent("label").parent("li").find('input:checkbox').prop("disabled", true).parent("label").parent("li").css({
			'color': '#c8c8c8'
		});
	};
	...
	$.fn.hummingbird.enableNode = function(tree, attr, name, state) {
		...
		//for a disabled unchecked node, set node checked and then trigger a click to uncheck
		//for a disabled checked node, set node unchecked and then trigger a click to check
		this_checkbox.prop("disabled", false).parent("label").parent("li").css({
			'color': '#636b6f'
		});
		//all parents enabled
		this_checkbox.parent("label").parent("li").parents("li").children("label").children("input[type='checkbox']").prop("disabled", false).parents("label").parent("li").css({
			'color': '#636b6f'
		});
		//all children enabled
		this_checkbox.parent("label").parent("li").find('input:checkbox').prop("disabled", false).parent("label").parent("li").css({
			'color': '#636b6f'
		});
		this_checkbox.prop("checked", state === false);
		nodeDisabled = false;
			...
			$(this).parent("label").parents("li").map(function() {
				...
				if (checkDisabled) {
					...
					if (not_disabled_sum == 0) {
						$(this).parent().parent().children("label").children("input[type='checkbox']").prop("disabled", true).parent("label").parent("li").css({
							'color': '#c8c8c8'
						});
					}
				}
